import cv2
import numpy as np
import glob

# 标定板的大小，标定板内角点的个数
CHECKERBOARD = (9,6)  # 每一列的角点和每一行的角点数
# 角点优化，迭代的终止条件
criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)

# step1:定义标定板在真实世界的坐标
# 创建一个向量来保存每张图片中角点的3D坐标
objpoints = []
# 创建一个向量来保存每张图片中角点的2D坐标
imgpoints = []

# 定义3D坐标:[row,col,z]
objp = np.zeros((1, CHECKERBOARD[0]*CHECKERBOARD[1], 3), np.float32)
objp[0,:,:2] = np.mgrid[0:CHECKERBOARD[0], 0:CHECKERBOARD[1]].T.reshape(-1, 2)

# step2:提取不同角度拍摄的图片
images = glob.glob('./images001/*.jpg')
images = sorted(images)
for i, fname in enumerate(images):
    img = cv2.imread(fname)  # 读取图片
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # RGB转换成灰度图

    # step3:计算标定板的角点的2D坐标
    # 寻找角点坐标，如果找到ret返回True, corners:[col, row]，原点在左上角
    ret, corners = cv2.findChessboardCorners(
        gray, CHECKERBOARD,
        cv2.CALIB_CB_ADAPTIVE_THRESH + cv2.CALIB_CB_FAST_CHECK + cv2.CALIB_CB_NORMALIZE_IMAGE
    )

    if ret == True:
        objpoints.append(objp)
        # 调用cornerSubPix对2D角点坐标位置进行优化
        corners2 = cv2.cornerSubPix(gray, corners, (11,11), (-1,-1), criteria)
        imgpoints.append(corners2)

        # 绘制寻找到的角点，从红色开始绘制，紫色结束
        img = cv2.drawChessboardCorners(img, CHECKERBOARD, corners2, ret)
        cv2.imshow(fname + " succeed", img)
    else:
        print(f"第{i}张图，{fname}未发现足够角点")
        cv2.imshow(fname + " failed", img)
    cv2.waitKey(1)
cv2.destroyAllWindows()

h, w = img.shape[:2]

# step4:相加标定
ret, mtx, dist, rvecs, tvecs = cv2.calibrateCamera(objpoints, imgpoints, gray.shape[::-1], None, None)

print("相机内参:")  # [[fx,0,cx],[0,fy,cy],[0,0,1]]
print(mtx, "\n")
print("畸变参数:")  # k1,k2,p1,p2,k3
print(dist, "\n")
print("旋转矩阵:")
print(rvecs, "\n")
print("平移矩阵:")
print(tvecs, "\n")

f_x = mtx[0, 0]
f_y = mtx[1, 1]
c_x = mtx[0, 2]
c_y = mtx[1, 2]

# 示例：使用 Rodrigues 转换为旋转矩阵
for img_idx, rvec in enumerate(rvecs):
    print(f"图像 {img_idx} 的旋转向量 (rvec):")
    print(rvec.flatten())  # 打印为一维数组，方便查看

    # 转换为旋转矩阵
    rotation_matrix, _ = cv2.Rodrigues(rvec)
    print(f"对应的旋转矩阵 (Rotation Matrix):")
    print(rotation_matrix)

# 假设 rvecs 是从 cv2.calibrateCamera 得到的旋转向量列表
# rvecs 是一个列表，每个元素是一个 (3, 1) 的数组
# 示例：rvecs = [rvec1, rvec2, ..., rvecN]

# 获取最后一个旋转向量
last_rvec = rvecs[-1]

# 使用 Rodrigues 公式将旋转向量转换为旋转矩阵
rotation_matrix, _ = cv2.Rodrigues(last_rvec)

# 将旋转矩阵的每个元素分别赋值给新的变量
r11, r12, r13 = rotation_matrix[0, 0], rotation_matrix[0, 1], rotation_matrix[0, 2]
r21, r22, r23 = rotation_matrix[1, 0], rotation_matrix[1, 1], rotation_matrix[1, 2]
r31, r32, r33 = rotation_matrix[2, 0], rotation_matrix[2, 1], rotation_matrix[2, 2]

# 打印赋值结果
print(f"r11: {r11}, r12: {r12}, r13: {r13}")
print(f"r21: {r21}, r22: {r22}, r23: {r23}")
print(f"r31: {r31}, r32: {r32}, r33: {r33}")

# 假设 tvecs 是从 cv2.calibrateCamera 得到的平移向量列表
# tvecs 是一个列表，每个元素是一个 (3, 1) 的数组
# 示例：tvecs = [tvec1, tvec2, ..., tvecN]

# 遍历每个平移向量
for img_idx, tvec in enumerate(tvecs):
    print(f"图像 {img_idx} 的平移向量 (tvec):")
    print(tvec.flatten())  # 打印为一维数组，方便查看

    # 将平移向量的每个元素分别赋值给新的变量
    tx, ty, tz = tvec.flatten()

    # 打印赋值结果
    print(f"tx: {tx}, ty: {ty}, tz: {tz}")

# step5:去畸变
img = cv2.imread('./images2/left12.jpg')
h, w = img.shape[:2]
newcameramtx, roi = cv2.getOptimalNewCameraMatrix(mtx, dist, (w, h), 1, (w, h))

# 使用 undistort 矫正图像
dst = cv2.undistort(img, mtx, dist, None, newcameramtx)
# 图片裁剪
x, y, w, h = roi
dst2 = dst[y:y+h, x:x+w]
print(f"ROI: x:{x},y:{y},w:{w},h:{h}")
cv2.imshow("original image", img)
cv2.imshow("image undistorted1", dst)
cv2.imshow("image undistorted1 ROI", dst2)
cv2.imwrite('./images2/undistorted/img_undistort.png', dst2)

# 使用 remapping
mapx, mapy = cv2.initUndistortRectifyMap(mtx, dist, None, newcameramtx, (w, h), 5)
dst = cv2.remap(img, mapx, mapy, cv2.INTER_LINEAR)

# crop the image
x, y, w, h = roi
dst2 = dst[y:y+h, x:x+w]  # x，宽，y，高
cv2.imshow("image undistorted2", dst)
cv2.imshow("image undistorted2 ROI", dst2)
cv2.imwrite('./images2/undistorted/img_remap.png', dst2)
cv2.waitKey(0)
cv2.destroyAllWindows()

# step6:计算重投影误差：
mean_error = 0
for i in range(len(objpoints)):
    # 使用内外参和畸变参数对点进行重投影.
    imgpoints2, _ = cv2.projectPoints(objpoints[i], rvecs[i], tvecs[i], mtx, dist)
    error = cv2.norm(imgpoints[i], imgpoints2, cv2.NORM_L2)/len(imgpoints2)  # L2范数，均方误差
    mean_error += error

mean_error /= len(objpoints)
print("total error: {}".format(mean_error))